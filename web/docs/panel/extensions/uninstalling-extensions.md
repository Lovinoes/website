...
- **Upgrading to a new version** - no need to uninstall the old one first. On Docker, upload the new `.c7s.zip` or overwrite the archive in `./build/extensions`; in the development environment run `panel-rs extensions update path/to/extension.c7s.zip`, since `extensions add` refuses a package that is already installed. The Panel handles the swap, and the extension's own migration scripts handle any schema changes between versions.
