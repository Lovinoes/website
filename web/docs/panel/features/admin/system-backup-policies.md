...
The **Backups** tab lists every backup the policy has taken, searchable: Name, Server, Node, Checksum, Size, Files, Retention, and Created, each row tagged with a **SYSTEM** badge and carrying a download action. **Retention** works as it does on a server's [System Backups](../server/backups.md#system-backups) tab: a badge for the rule keeping the backup longest, and the scheduled run at which the policy is expected to drop it. A database policy adds a **Kind** column, and its rows report no file count because a dump is a single file. Policy backups do not count towards each server's backup limit and no [backup group](../server/backups.md#backup-groups) touches them; the policy's own **Retention** rules manage them instead, re-checked once an hour. Unlocked failed attempts are cleaned up 24 hours after they finish.
