...
Deleting a backup runs `restic forget` for its snapshot, and that is all: the data the snapshot referenced stays in the repository until `restic prune` runs. **Prune Jobs** schedule that. Each job (up to 50 per configuration) has a **Cron Schedule** in six-field form (`second minute hour day month weekday`, e.g. `0 0 3 * * *` for 03:00 daily) and the **Nodes** it runs on. The panel checks every minute and asks each selected node to start the prune in the background; one node per repository is enough, since they all prune the same data.

::: warning Prune jobs only dry-run for now
This is a known bug: the panel currently starts scheduled prunes with `--dry-run`, so restic only reports what it would remove and nothing is reclaimed. Since Wings 1.2.2 deleting a backup no longer prunes either, so until the fix ships, prune the repository by hand as described under [Troubleshooting](#troubleshooting).
:::
...
| Restic storage keeps growing even though old backups are deleted | Deleting a backup only forgets the snapshot. The data stays until `restic prune` runs, and [prune jobs](#disk-specific-settings) currently only do a dry run. Turn on **Maintenance Enabled** for the configuration, run `restic prune` against the repository (and `restic unlock` first if a stale lock is reported), then turn maintenance off. |
